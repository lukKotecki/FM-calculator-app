...
function App() {
  ...
  function calculateResult(concatedNumbers) {
    ...
    for (let i = 0; i < secondArray.length; i++) {
      if (secondArray[i] === "+") {
        secondArray[i + 1] =
          Number(secondArray[i - 1]) + Number(secondArray[i + 1]);
  ...
  function setCharToInput(value) {
    ...
    if (value === "-") {
      if (previousInput === "-") {
        //skip if previous was minus
        return;
      }
      if (previousInput === "+" || (input.length === 1 && isNaN(input[0]))) {
        // replace plus
